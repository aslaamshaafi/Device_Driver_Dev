...
#include <asm/ioctl.h>
// good link to learn the basics of ioctl (https://embetronicx.com/tutorials/linux/device-drivers/ioctl-tutorial-in-linux/)
...
/* BOOTS_FILL_ZERO: to fill our character array with zeros
...
*/
#define BOOST_FILL_ZERO		_IO( BOOST_MAGIC, 1 )

/* BOOST_FILL_ZERO: to fill our character array with a character
   As we can see there is not data transfer. The moment
   our driver gets this command, it executes the following
   for loop
...
*/
#define BOOST_FILL_CHAR		_IOW( BOOST_MAGIC, 2, char )

/* BOOST_SET_SIZE: to set the size of the character array */

#define BOOST_SET_SIZE		_IOW( BOOST_MAGIC, 3, uint )

/* BOOST_GET_SIZE: to get the size of the character array */
#define BOOST_GET_SIZE		_IOR( BOOST_MAGIC, 4, uint )

#define WRITE_VALUE 			_IOW(BOOST_MAGIC,5,char*)
#define READ_VALUE 					_IOR(BOOST_MAGIC,6,char*)
#define BOOST_MAX_CMDS		6
